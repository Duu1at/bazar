...
void main() {
  ...
  group('SecureStorage', () {
    ...
    group('read', () {
      test('returns value when FlutterSecureStorage returns successfully', () async {
        when(() => flutterSecureStorage.read(key: any(named: 'key'))).thenAnswer((_) => Future.value(mockValue));
      ...
      test('returns null when FlutterSecureStorage returns null', () async {
        when(() => flutterSecureStorage.read(key: any(named: 'key'))).thenAnswer((_) => Future<String?>.value());
        final actual = await secureStorage.read(key: mockKey);
        expect(actual, isNull);
      });

      test('throws StorageException when FlutterSecureStorage fails', () async {
        when(() => flutterSecureStorage.read(key: any(named: 'key'))).thenThrow(mockException);
    ...
    group('delete', () {
      test('completes when FlutterSecureStorage completes', () async {
        when(() => flutterSecureStorage.delete(key: any(named: 'key'))).thenAnswer((_) => Future.value());
        expect(
      ...
      test('throws StorageException when FlutterSecureStorage fails', () async {
        when(() => flutterSecureStorage.delete(key: any(named: 'key'))).thenThrow(mockException);
        try {
